...
from torch._inductor.config import expand_dimension_for_pointwise_nodes 
...
class ModelWrapper:
    ...
    def score_window(self, window_values: np.ndarray) -> float: 
        ...
        w = np.asanyarray(window_values, dtype=float).reshape(-1, 1) 
    ...
    def score_series(self, series: np.ndarray, window: Optional[int] = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        window = np.lib.stride_tricks.sliding_window_view(x_scaled, window_shape=window)
        batch = torch.tensor(window, dtype=torch.float32).unsqueeze(-1).to(self.device) 
        with torch.no_grad(): 
            recon = self.model(batch)
