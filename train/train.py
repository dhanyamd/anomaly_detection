"""Train LSTM Autoencoder for univariate time series anomaly detction"""
import os 
import argparse
...
class WindowDataset(Dataset): 
    ...
    def __getitem__(self, idx):
        ...
        return torch.Tensor(w, dtype=torch.float32).unsqueeze(-1) 
    
def load_series(path: str, value_col: str ="value"): 
    df = pd.read_csv(path) 
    if value_col not in df.columns: 
        df.columns = ["timestamps", "value"] 
...
def train(cfg: dict): 
    ...
    optimizer = optim.Adam(model.parameters(), lt=float(tr_cfg["lr"])) 
